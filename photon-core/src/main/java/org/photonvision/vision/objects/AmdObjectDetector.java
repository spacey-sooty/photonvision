...
public class AmdObjectDetector implements ObjectDetector {
    private static final Logger logger = new Logger(AmdObjectDetector.class, LogGroup.General);
    ...
    public AmdObjectDetector(AmdModel model, Size inputSize) {
        ...
        if (objPointer < 0) {
            throw new RuntimeException(
                    "Failed to create AMD detector from path "
                            + model.modelFile.getPath()
                            + " pointer: "
                            + objPointer);
    ...
    public List<NeuralNetworkPipeResult> detect(Mat in, double nmsThresh, double boxThresh) {
        ...
        // Letterbox scale = Letterbox.letterbox(in, letterboxed, this.inSize,
        // ColorHelper.colorToScalar(Color.GRAY));

        // if (!letterboxed.size().equals(this.inSize)) {
        //   letterboxed.release();
        //   throw new RuntimeException("Letterboxed image has incorrect size! Expected " + this.inSize
        // + " but got " + letterboxed.size());
        // }
