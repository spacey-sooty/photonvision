...
import org.photonvision.amd.AmdJNI;
import org.photonvision.vision.pipe.impl.NeuralNetworkPipeResult;

/** Manages an object detector using the rubik backend. */
public class OnnxObjectDetector implements ObjectDetector {
    private static final Logger logger = new Logger(OnnxObjectDetector.class, LogGroup.General);
     ...
     */
    public OnnxObjectDetector(OnnxModel model, Size inputSize) {
        this.model = model;
        this.inputSize = inputSize;

        // Create the detector
        try {
            ptr = AmdJNI.create(model.modelFile.getPath().toString());
        } catch (Exception e) {
        ...
        if (!isValid()) {
            logger.error(
                    "Failed to create detector from path "
                            ...
                            + ". Please ensure the model is valid and compatible with the AMD backend.");
            throw new RuntimeException(
                    "Failed to create detector from path " + model.modelFile.getPath());
        } else if (!AmdJNI.isQuantized(ptr)) {
    ...
    public List<NeuralNetworkPipeResult> detect(Mat in, double nmsThresh, double boxThresh) {
        ...
        var results = AmdJNI.detect(ptr, letterboxed.getNativeObjAddr(), boxThresh, nmsThresh);
