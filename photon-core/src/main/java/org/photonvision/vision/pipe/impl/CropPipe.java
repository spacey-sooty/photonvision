 ...
 *
 * <p>For static cropping, build the rectangle from the UI's pixel ranges with {@link
 * #staticCropRect}; for ML-assisted tag detection, pass the detection's bounding box.
 */
public class CropPipe extends CVPipe<CVMat, CVMat, CropPipe.CropPipeParams> {
    ...
    /**
     * Build the static crop rectangle from the UI's pixel ranges in the pipeline settings. The ranges
     * are stored as [min, max] pixel couples.
     *
     ...
     *     degenerate.
     */
    public static Rect staticCropRect(AdvancedPipelineSettings settings) {
    ...
    /**
     * For apriltag pipelines, grow the region up to the tile boundary below its origin rather than
     * moving it, so the crop still covers everything that was asked for. Only the apriltag detector
     * cares about the tiling, so other pipelines get the rectangle they asked for.
     */
